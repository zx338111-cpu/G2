...
class RackLidarDockingController:
    ...
    def cancel_blocking_task(self):
        ...
        # 0/3/7/9 是当前实机上常见的非运行/结束类状态，不需要 cancel。
    ...
    def read_rack_distance(self):
        ...
        valid = (
            ...
            & (forward_coord >= self.min_range_m)
            & (forward_coord <= self.max_range_m)
            & (np.abs(lateral_coord) <= self.lateral_half_width_m)
            & (vertical_coord >= self.z_min_m)
            & (vertical_coord <= self.z_max_m)
        )

        forward = forward_coord[valid]
        if len(forward) < self.min_cluster_points:
        ...
        bins = np.arange(
            self.min_range_m,
            self.max_range_m + self.bin_width_m,
            self.bin_width_m,
        )
