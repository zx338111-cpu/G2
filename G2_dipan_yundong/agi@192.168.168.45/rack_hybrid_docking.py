...
class RackHybridDockingController:
    ...
    def approach_to_rack(
        ...
        final_stop_mm:
            期望停稳后的距离，单位 mm。默认 500 表示最终停在料架前 0.5m。
        final_brake_margin_mm:
        ...
        if coarse_keepalive_min_lidar_m is None:
            ...
            #   - switch_ultrasonic_mm/1000 + 0.2：离超声切换区至少 0.2m。
            coarse_keepalive_min_lidar_m = max(
                coarse_stop_m + 0.4,
                switch_ultrasonic_mm / 1000.0 + 0.2,
            )
        ...
        while time.time() < initial_deadline:
            ...
            if (
                latest_ultrasonic_filtered is not None
                and len(ultrasonic_history) >= ultrasonic_history_size
                and latest_ultrasonic_filtered <= switch_ultrasonic_mm
            ):
